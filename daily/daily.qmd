...
# Handle report start/end time
date_range = get_report_start_end(report_dir, "daily", run_future = FALSE)

hours_to_summarise = seq(date_range[1], date_range[2], "1 hours")

caption_range = paste(
  date_range[1] |> format("%b %d, %Y %H:%M UTC"),
  "to",
  date_range[2] |> format("%b %d %H:%M UTC")
)
plot_captions = paste(
  "Obs. from Canadian",
...
obs = load_report_data(date_range[1], date_range[2], max_pm25_allowed) |>
  dplyr::mutate(prov_terr = factor(prov_terr, prov_pretty, names(prov_pretty)))

.tmp = make_summaries(obs, type = "daily")
...
<center><h3>Non-validated Data - <mark class="bg-info">`r max(obs$date) |> format("%Y-%m-%d %H UTC")`</mark></center>
<center><h4>`r max(obs$date) |> lubridate::with_tz('America/Vancouver') |>format("%b %d, %I %p")` in Vancouver --- `r max(obs$date) |> lubridate::with_tz('America/Halifax') |>format("%b %d, %I %p")` in Halifax</h4></center>
...
There are <strong>`r sum(p_fem$n)` FEM sites</strong> and <strong>`r sum(p_pa$n)` PA sites</strong> currently reporting PM<sub>2.5</sub> in Canada. <strong>

- `r  p_fem$p[4]`%</strong> of the FEM sites in Canada have a 24-hour mean exceeding 100 &mu;g m<sup>-3</sup>, <strong> `r p_fem$p[3]`%</strong> are between 60 and 100 &mu;g m<sup>-3</sup>, and <strong> `r p_fem$p[2]`%</strong> are between 30 and 60 &mu;g m<sup>-3</sup>.<strong> 

- `r p_pa$p[4]`%</strong> of the PA sites in Canada have a 24-hour mean exceeding 100 &mu;g m<sup>-3</sup>, <strong> `r p_pa$p[3]`%</strong> are between 60 and 100 &mu;g m<sup>-3</sup>, and <strong> `r p_pa$p[2]`%</strong> are between 30 and 60 &mu;g m<sup>-3</sup>.
...
plot_paths$mean[['FEM and PA']] |>
  ...
  plot_card(
    plot_text$mean |>
      stringr::str_replace("%m", "regulatory (FEM) and PurpleAir (PA)")
  )
```
...
# Make data for the maps
map_data = make_map_data(obs_summary)

# Get bounds to be used for all maps
bounds = map_data |>
  dplyr::ungroup() |>
...
.tmp = lapply(monitor_groups, \(m) {
  ...
  if (m != "FEM and PA") {
    pd = pd |>
      subset(monitor == names(monitor_groups)[monitor_groups == m])
  }
  pd |>
    make_map(bounds, zone_summaries, include_active_fires = FALSE) |>
    mapview::mapshot(plot_paths[[m]], libdir = "./libs", selfcontained = FALSE)
})

# Define plot summary text (figure titles essentially)
plot_texts = paste0(
  "Here is an interactive map showing 24-hour mean PM<sub>2.5</sub> concentrations from monitoring sites (",
  monitor_groups,
  ") in Canada for the past 24 hours. Canadian forecast zones are shown coloured by the mean 24-hour concentration of the (",
  monitor_groups,
  ") monitors within the zone. Toggle layers displayed in the top right menu.
...
table_data = make_map_table_data(obs_summary, obs_current)

# Make tables
tables = lapply(names(monitor_groups), \(m) {
  if (m != names(monitor_groups)[3]) {
    pd = table_data |>
      subset(Monitor == m)
  } else {
    pd = table_data
...
# Make download buttons linked to csv copy of data in tables
dl_buttons = lapply(names(monitor_groups), \(m) {
  ...
  # MAke data path
  outdir = paste0(report_dir, "data/")
  outdir2 = "./data/"
  # Make data file name
  file = "pm2.5_monitor_sites" |>
    paste(
      m |> stringr::str_to_lower() |> stringr::str_replace_all(" ", "_"),
      max(obs$date) |> format("%Y-%m-%d_%HZ"),
      sep = "_"
    ) |>
    paste0(".csv")
  # Write data to csv
  ...
  pd |>
    setNames(names(pd) |> stringr::str_replace_all("\n", " ")) |>
    data.table::fwrite(paste0(outdir, file), dateTimeAs = "write.csv")
  # Return html for button linked to csv
  dl_button_html(outdir2, file)
}) |>
  setNames(monitor_groups)
...
aqhi_p_counts_by_prov = obs_summary |>
  dplyr::group_by(
    aqhi_p_24hr = aqhi_p(pm25_mean, use_risk = TRUE, include_ugm3_range = TRUE),
    prov_terr,
    monitor
  ) |>
...
aqhi_p_counts_by_prov = aqhi_p_counts_by_prov |>
  dplyr::full_join(
    expand.grid(
      aqhi_p_24hr = levels(aqhi_p_counts_by_prov$aqhi_p_24hr),
      prov_terr = names(prov_pretty)
    ),
    by = c("aqhi_p_24hr", "prov_terr")
  ) |>
  dplyr::mutate(dplyr::across(c("FEM", "PA"), \(x) {
...
There was <strong>`r aqhi_p_counts$FEM[4]` FEM monitors</strong> and <strong>`r aqhi_p_counts$PA[4]` PA monitors</strong> in Canada with a 24-hour mean PM<sub>2.5</sub> concentration <strong>exceeding 100 &mu;g m<sup>-3</sup></strong>
...
There was <strong>`r aqhi_p_counts$FEM[3]` FEM monitors</strong> and <strong>`r aqhi_p_counts$PA[3]` PA monitors</strong> in Canada with a 24-hour mean PM<sub>2.5</sub> concentration <strong>between 60 and 100 &mu;g m<sup>-3</sup></strong>
...
pd = obs_summary |>
  dplyr::mutate(
    monitor2 = ifelse(
      monitor == "FEM",
      monitor_groups[1],
      ifelse(monitor == "PA", monitor_groups[2], monitor_groups[3])
    )
  ) |>
  dplyr::bind_rows(
    obs_summary |> dplyr::mutate(monitor2 = monitor_groups[3])
  ) |>
...
.tmp = lapply(monitor_groups, \(m) {
  ...
  fills = 1:length(leg_ugm3[[1]]) |>
    lapply(\(i) {
      max_val = ceiling(max(pdd$pm25_mean, na.rm = TRUE) / 10) * 10
  ...
  box = boxplot(pm25_mean ~ prov_terr, pdd, plot = FALSE)
  pdd = pdd |>
    dplyr::mutate(is_outlier = pm25_mean > box$stats[4, as.numeric(prov_terr)])
  outliers = subset(pdd, is_outlier)

  pp = pdd |>
    plot_ly(y = ~ round(pm25_mean, 1), x = ~prov_terr) |>
    add_boxplot(
      name = "",
      boxpoints = FALSE,
      color = I('black'),
    ...
    layout(
      title = paste(
        ...
        max(obs$date) |> format("%Y-%m-%d %H:%M (UTC)"),
        "to",
        max(obs$date) |> format("%Y-%m-%d %H:%M (UTC)")
      ),
      shapes = fills,
      ...
      yaxis = list(
        title = paste0("24-hour Site Mean PM2.5 (ug/m3; ", m, ")"),
        zeroline = TRUE,
        showgrid = FALSE
      )
    ) |>
...
`r vals[[1]]$M` (a `r vals[[1]]$T` monitor located `r vals[[1]]$D` km from `r vals[[1]]$C`) in `r vals[[1]]$P` had the highest observed hourly PM<sub>2.5</sub> concentration in Canada from the FEM network in the past 24 hours (`r vals[[1]]$PM` &mu;g m<sup>-3</sup>). During this period, `r vals[[2]]$M` (a `r vals[[2]]$T` monitor located `r vals[[2]]$D` km from `r vals[[2]]$C`) in `r vals[[2]]$P` had the highest 24-hour mean in Canada from the FEM network (`r round(vals[[2]]$PM,1)`&mu;g m<sup>-3</sup>).

`r vals[[3]]$M` (a `r vals[[3]]$T` monitor located `r vals[[3]]$D` km from `r vals[[3]]$C`) in `r vals[[3]]$P` had the highest observed hourly PM<sub>2.5</sub> concentration in Canada from the PA network in the past 24 hours (`r vals[[3]]$PM` &mu;g m<sup>-3</sup>). During this period, `r vals[[4]]$M` (a `r vals[[4]]$T` monitor located `r vals[[4]]$D` km from `r vals[[4]]$C`) in `r vals[[4]]$P` had the highest 24-hour mean in Canada from the PA network (`r round(vals[[4]]$PM,1)`&mu;g m<sup>-3</sup>).
...
grid_data = list(
  median = make_grid_data(obs, median),
  maximum = make_grid_data(obs, max)
)

# Make paths to plot files
plot_paths = report_dir |>
  paste0(
    "plots/prov_median-peak_hourly_grid_",
    monitor_groups |>
      stringr::str_to_lower() |>
      stringr::str_replace_all(" ", "_"),
    '_',
...
.tmp = names(monitor_groups) |>
  lapply(\(m) {
    ...
    (ggs[[1]] + ggs[[2]]) |>
      ggsave(filename = pth, height = 4, width = 11, units = 'in', dpi = 300)
  }) |>
  setNames(monitor_groups)

# Define plot summary text (figure titles essentially)
plot_texts = paste0(
  "Median [Left] and maximum [Right] hourly PM<sub>2.5</sub> concentrations for all ",
  names(monitor_groups),
  " monitoring sites in each province/territory. The median is useful for identifying large scale impacts across the province/territory, while the maximum can be used to identify impacts in at least one area of the province/territory."
) |>
  setNames(monitor_groups) |>
  as.list()
...
`r join_list_sentence(p$y)` had a significant amount of locations which experienced elevated PM<sub>2.5</sub> concentrations  (> 30 &mu;g m<sup>-3</sup>) for at least 3 of the past 24 hours [see median plot]. 

`r join_list_sentence(p2$y)` had at least one location which experienced elevated PM<sub>2.5</sub> concentrations (> 30 &mu;g m<sup>-3</sup>) for at least 3 of the past 24 hours [see maximum plot]. 
...
grid_data = make_grid_data(obs, median, type = "daily", fcst_zones)

# Make paths to plot files
plot_paths = paste0(
  report_dir,
  "plots/zone_median_hourly_grid_",
  unlist(prov_order),
  '_',
  format(date_range[2], "%F-%Hz"),
  ".png"
...
plots = lapply(unlist(prov_order), \(p) {
  ...
  pd |>
    ...
    ggsave(filename = pth, height = 7, width = 11, units = 'in', dpi = 300)
}) |>
  setNames(unlist(prov_order))

# Define plot summary text (figure titles essentially)
plot_texts = paste0(
  "Hourly median PM<sub>2.5</sub> concentrations for all monitors (FEM and PA) within each forecast zone for ",
  unlist(prov_order),
  ". A white cell indicates there was no data for that hour from any monitor within that zone. Zones are grouped into those with and those without data and are sorted alphabetically."
) |>
  setNames(unlist(prov_order)) |>
  as.list()


### NOT SURE IF THIS CODE IS USED ANYMORE

# fcst_zone_totals = fcst_zones |>
#   data.frame() |>
#   dplyr::bind_rows(fcst_zones |> subset(PROVINCE_C |> stringr::str_detect(","))) |>
#   dplyr::mutate(PROVINCE_C = ifelse(duplicated(NAME),
#                              (PROVINCE_C |> stringr::str_split(",", simplify = TRUE))[,2],
#                              (PROVINCE_C |> stringr::str_split(",", simplify = TRUE))[,1])) |>
#   dplyr::group_by(PROVINCE_C) |>
#   dplyr::summarise(total_zones = n()) |>
#   dplyr::rename(prov = PROVINCE_C) |>
#   dplyr::mutate(prov=ifelse(prov == "YT","YK",prov))

# pd2 = obs |>
#   dplyr::group_by(y = fcst_zone, x= hour(date)|>factor(unique(hour(date))), z = prov_terr, monitor)|>
#   dplyr::summarise(fill = quantile(pm25,0.5,na.rm=TRUE) |> aqhi_p()) |>
#   dplyr::full_join(expand.grid(y = obs$fcst_zone |> unique(),
#                         x = hour_order |>factor(hour_order)) |>
#               dplyr::left_join(subset(obs, !duplicated(fcst_zone)) |>
#                           dplyr::select(y = fcst_zone, z = prov_terr),
#                         by = "y"),
#             by = c('y','x','z')) |>
#   dplyr::group_by(monitor, z,y) |>
#   dplyr::summarise(n = n()) |>
#   dplyr::group_by(monitor, prov = z) |>
#   dplyr::summarise(n_fem_only = n()) |>
#   subset(monitor == "FEM")

# p = pd |>
#   subset(!all_missing & y != "Not inside a zone")  |>
#   dplyr::group_by(prov = z) |>
#   dplyr::summarise(n_covered = n()/24)|>
#   dplyr::full_join(data.frame(prov = names(prov_pretty)), by = "prov")  |>
#   dplyr::left_join(fcst_zone_totals, by = "prov") |>
#   dplyr::left_join(pd2, by = "prov") |>
#   dplyr::mutate(p = round(n_covered / total_zones * 100) |> paste0("%"),
#          p_fem = round(n_fem_only / total_zones * 100) |> paste0("%"),
#          n_covered = ifelse(is.na(n_covered),0,n_covered)) |>
