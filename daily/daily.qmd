...
summaries <- obs |>
  make_summaries(type = "daily", fcst_zones = fcst_zones, meta_cols = meta_cols)
...
labels = summaries$overall |>
  dplyr::bind_rows(
    summaries$overall |> dplyr::mutate(monitor = "FEM and PA")
  ) |>
  dplyr::full_join(
    expand.grid(
...
for (stat in c("mean", "max")) {
  ...
  pd[[stat]] = make_donut_data(summaries$overall, paste0("pm25_", stat))
...
map_data = make_map_data(summaries$overall)
...
.tmp = lapply(monitor_groups, \(m) {
  ...
  pd |>
    make_map(bounds, summaries$by_zone, include_active_fires = FALSE) |>
...
table_data = make_map_table_data(summaries$overall, summaries$current)
...
# Make summary data for autmated text for this section
aqhi_p_counts_by_prov = summaries$overall |>
  dplyr::group_by(
...
pd = summaries$overall |>
  ...
  dplyr::bind_rows(
    summaries$overall |> dplyr::mutate(monitor2 = monitor_groups[3])
  ) |>
