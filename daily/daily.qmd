...
summaries <- obs |> make_summaries(type = "daily", fcst_zones = fcst_zones)
...
labels = summaries$obs_summary |>
  dplyr::bind_rows(summaries$obs_summary |> dplyr::mutate(monitor = "FEM and PA")) |>
  dplyr::full_join(
    expand.grid(
...
for (stat in c("mean", "max")) {
  ...
  pd[[stat]] = make_donut_data(summaries$obs_summary, paste0("pm25_", stat))
...
map_data = make_map_data(summaries$obs_summary)
...
.tmp = lapply(monitor_groups, \(m) {
  ...
  pd |>
    make_map(bounds, summaries$zone_summaries, include_active_fires = FALSE) |>
...
table_data = make_map_table_data(summaries$obs_summary, summaries$obs_current)
...
# Make summary data for autmated text for this section
aqhi_p_counts_by_prov = summaries$obs_summary |>
  dplyr::group_by(
...
pd = summaries$obs_summary |>
  ...
  dplyr::bind_rows(
    summaries$obs_summary |> dplyr::mutate(monitor2 = monitor_groups[3])
  ) |>
...
# Make data for table
table_data = summaries$community_summ |>
