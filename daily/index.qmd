...
```{js onpageload}
var icon_dir = "index_files/icons/";
var icons = [
  icon_dir + "icons8-doughnut-chart-64.png",
  icon_dir + "icons8-map-64.png",
  icon_dir + "icons8-vertical-timeline-64.png",
  icon_dir + "icons8-data-sheet-64.png",
  icon_dir + "icons8-grid-64.png"
];
var main_header = "#analysis-of-past-24-hours-of-pm2\\.5-observations";
document.onload(() => on_page_load(main_header, icons));  
```
...
boxplot_vals <- boxplots$summary_values
...
## Analysis of Past 24-Hours of PM<sub>2.5</sub> Observations {style="display:none;"}

*Select your question from the following tabs*
...
There are <strong>`r sum(prov_donuts_text$p$fem$n)` FEM sites</strong> and <strong>`r sum(prov_donuts_text$p$pa$n)` PA sites</strong> currently reporting PM<sub>2.5</sub> in Canada. <strong>

- `r prov_donuts_text$p$fem$p[4]`%</strong> of the FEM sites in Canada have a 24-hour mean exceeding 100 &mu;g m<sup>-3</sup>, <strong> `r prov_donuts_text$p$fem$p[3]`%</strong> are between 60 and 100 &mu;g m<sup>-3</sup>, and <strong> `r prov_donuts_text$p$fem$p[2]`%</strong> are between 30 and 60 &mu;g m<sup>-3</sup>.<strong> 

- `r prov_donuts_text$p$pa$p[4]`%</strong> of the PA sites in Canada have a 24-hour mean exceeding 100 &mu;g m<sup>-3</sup>, <strong> `r prov_donuts_text$p$pa$p[3]`%</strong> are between 60 and 100 &mu;g m<sup>-3</sup>, and <strong> `r prov_donuts_text$p$pa$p[2]`%</strong> are between 30 and 60 &mu;g m<sup>-3</sup>.
...
`r boxplot_vals$FEM[[1]]$M` (a `r boxplot_vals$FEM[[1]]$T` monitor
located `r boxplot_vals$FEM[[1]]$D` km from `r boxplot_vals$FEM[[1]]$C`)
in `r boxplot_vals$FEM[[1]]$P` had the highest observed hourly PM<sub>2.5</sub>
concentration in Canada from the FEM network in the past 24 hours (`r boxplot_vals$FEM[[1]]$PM` &mu;g m<sup>-3</sup>).
During this period, `r boxplot_vals$FEM[[2]]$M`
(a `r boxplot_vals$FEM[[2]]$T` monitor located `r boxplot_vals$FEM[[2]]$D` km
from `r boxplot_vals$FEM[[2]]$C`) in `r boxplot_vals$FEM[[2]]$P`
had the highest 24-hour mean in Canada from the FEM network (`r round(boxplot_vals$FEM[[2]]$PM,1)`&mu;g m<sup>-3</sup>).

`r boxplot_vals$PA[[1]]$M` (a `r boxplot_vals$PA[[1]]$T` monitor
located `r boxplot_vals$PA[[1]]$D` km from `r boxplot_vals$PA[[1]]$C`)
in `r boxplot_vals$PA[[1]]$P` had the highest observed hourly PM<sub>2.5</sub>
concentration in Canada from the PA network in the past 24 hours (`r boxplot_vals$PA[[1]]$PM` &mu;g m<sup>-3</sup>).
During this period, `r boxplot_vals$PA[[2]]$M`
(a `r boxplot_vals$PA[[2]]$T` monitor located `r boxplot_vals$PA[[2]]$D` km
from `r boxplot_vals$PA[[2]]$C`) in `r boxplot_vals$PA[[2]]$P`
had the highest 24-hour mean in Canada from the PA network (`r round(boxplot_vals$PA[[2]]$PM,1)`&mu;g m<sup>-3</sup>).
...
`r join_list_sentence(prov_grid_texts$p$median$y)` had a significant amount of locations
which experienced elevated PM<sub>2.5</sub> concentrations  (> 30 &mu;g m<sup>-3</sup>)
for at least 3 of the past 24 hours [see median plot]. 

`r join_list_sentence(prov_grid_texts$p$maximum$y)` had at least one location
which experienced elevated PM<sub>2.5</sub> concentrations (> 30 &mu;g m<sup>-3</sup>)
for at least 3 of the past 24 hours [see maximum plot]. 
...
in Canada in the past 24 hours (`r community_table_vals[[1]]$PM2` &mu;g m<sup>-3</sup>). 
...
(`r round(community_table_vals[[2]]$PM1,1)` &mu;g m<sup>-3</sup>).
...
where at least one monitor exceeded 100 &mu;g m<sup>-3</sup> and `r community_table_vals[[3]]$H2` hours
where at least one monitor exceeded 60 &mu;g m<sup>-3</sup>.
