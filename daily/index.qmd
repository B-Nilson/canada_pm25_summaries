...
map_tables <- lapply(names(monitor_groups), \(monitor_group) {
  ...
  table_path <- "%s/%s_map_table_%s.html" |>
    sprintf(
      report_dir |> file.path(figure_dir),
      monitor_groups_cleaned[[monitor_group]],
      plot_timestamp
    )
  map_table$html |> htmlwidgets::saveWidget(table_path)
  map_table$path <- table_path
  return(map_table)
}) |>
  setNames(monitor_groups)

map_table_buttons <- map_tables |> lapply(\(x) x$dl_button)
map_tables <- map_tables |> lapply(\(x) x$path)
...
`r build_tabs(monitor_groups, map_paths, map_texts, report_dir, map_table_buttons, map_tables, iframe = TRUE)`
