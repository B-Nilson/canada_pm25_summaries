...
# Define plot summary text (figure titles essentially)
map_texts <- "Here is an interactive map showing 24-hour mean PM<sub>2.5</sub> concentrations from monitoring sites (%s) in Canada for the past 24 hours. 
...
Below the map is a table summarising the data from the monitoring sites displayed in the map." |>
  sprintf(monitor_groups) |>
  setNames(monitor_groups) |>
  as.list()

# Make and save tables
overall_summary_tables <- lapply(names(monitor_groups), \(monitor_group) {
  summaries$overall |>
    dplyr::filter(monitor == monitor_group | monitor_group == "FEM and PA") |>
...
overall_summary_table_buttons <- overall_summary_tables |>
  lapply(\(x) x$dl_button)
overall_summary_tables <- overall_summary_tables |> lapply(\(x) x$path)
...
community_table$card <- community_table$path |>
  ...
  plot_card(text = table_caption, iframe = TRUE) |>
  stringr::str_replace(
    "<iframe",
    paste0(community_table$dl_button, "\n<iframe")
  ) |>
  knitr::asis_output()
...
`r build_tabs(monitor_groups, map_paths, map_texts, report_dir, overall_summary_table_buttons, overall_summary_tables, iframe = TRUE)`
...
`r build_tabs(monitor_groups, boxplot_paths, boxplot_texts, report_dir, iframe = TRUE)`
