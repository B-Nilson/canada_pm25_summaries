...
report_season = lubridate::year(date_range[1]) |>
  paste(
    ifelse(
      lubridate::month(date_range[1]) %in% months_in_seasons$Summer, 
      "Summer", "Winter"
    )
  )
...
gg = daily_zone_means |>
  ...
  scale_x_datetime(expand = expansion(0), breaks = seq(date_range[1], date_range[2],"months"), date_labels = "%b.")+
  ggpubr::theme_pubr(border = TRUE) +
...
for(stat in names(pd)){
  ...
  gg=pd[[stat]] |>
    ...
    geom_vline(
      xintercept = seq(date_range[1], date_range[2], "months"), colour = 'black'
    ) + 
...
pd = obs |>
  ...
  dplyr::full_join(expand.grid(site_id_monitor = paste(meta$site_id, meta$monitor), 
                        date = seq(date_range[1], date_range[2], "1 hours")) |>
