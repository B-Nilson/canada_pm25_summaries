...
date_range = get_report_start_end(report_dir,type =  "seasonal",
                                  months_in_seasons = months_in_seasons)
min_date = date_range[1]
max_date = date_range[2]

report_season = lubridate::year(min_date) |>
  paste(
    ifelse(
      lubridate::month(min_date) %in% months_in_seasons$Summer, 
      "Summer", "Winter"
    )
  )
...
season_rds =  paste0(report_dir, "data/", report_season,"_obs.rds")

if(!file.exists(season_rds)){
  obs = load_report_data(min_date, max_date, max_pm25_allowed)|>
    # Infill Date Gaps
    dplyr::group_by(site_id, monitor, lat, lng, name, is_aqsu, prov_terr, fcst_zone,
             nearest_community, nc_dist_km, nc_lat, nc_lng) |>
    tidyr::complete(date = hours_to_summarise)
  # Save for testing
  saveRDS(obs, season_rds)
}else{
  obs = readRDS(season_rds)
}
...
<center><h3>`r format(min_date,"%B %Y")` to `r format(max_date,"%B %Y")`</h3></center>
...
seasonal_zone_summaries = obs |>
  ...
  dplyr::full_join(fcst_zones |> dplyr::select(fcst_zone = NAME), by = "fcst_zone") |>
  dplyr::mutate(
...
gg = daily_zone_means |>
  ...
  scale_x_datetime(expand = expansion(0), breaks = seq(min_date, max_date,"months"), date_labels = "%b.")+
  ggpubr::theme_pubr(border = TRUE) +
...
fcst_zones_clean = fcst_zones |>
  data.frame() |>
  dplyr::select(-geometry, -fcst_zones_fr) 

fcst_zones_clean = fcst_zones_clean |>
  dplyr::mutate(prov_terr = stringr::str_replace_all(prov_terr, ",.*", "")) |>
  dplyr::bind_rows(
    fcst_zones_clean |> 
      dplyr::filter(stringr::str_detect(prov_terr, ",")) |>
      dplyr::mutate(prov_terr = stringr::str_replace_all(prov_terr, ".*,", ""))
  )
...
pd$max = test_data |>
  ...
  dplyr::summarise(n = n()) |>
  dplyr::full_join(fcst_zones_clean |> 
              dplyr::group_by(prov_terr = unlist(prov_pretty[prov_terr])) |>
              dplyr::summarise(n_zones = n()), 
            by = "prov_terr") |>
  dplyr::ungroup() |>
...
for(stat in names(pd)){
  ...
  gg=pd[[stat]] |>
    ...
    geom_vline(
      xintercept = seq(min_date, max_date, "months"), colour = 'black'
    ) + 
...
table_data = events |>
  ...
  dplyr::mutate(
    # min_date = format(min_date, "%m (%b.) %d"),
         # max_date = format(max_date, "%m (%b.) %d"),
...
pd = obs |>
  ...
  dplyr::full_join(expand.grid(site_id_monitor = paste(meta$site_id, meta$monitor), 
                        date = seq(min_date, max_date, "1 hours")) |>
  ...
  dplyr::filter(lubridate::floor_date(date, "days") |> dplyr::between(min_date, max_date)) |>
  dplyr::mutate(event_range = ifelse(min_date != max_date,
                              paste(format(min_date, "%b. %d"), "-", format(max_date, "%b. %d")),
                              format(min_date, "%b. %d")))
