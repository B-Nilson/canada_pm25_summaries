---
params:
  report: !expr source("../R/set_params.R"); set_params(type = "seasonal")
...
old_report_dropdown <- report_dir |>
  handle_old_reports(report_pattern = "[summer,winter]\\.html$")
...
monitor_count_text = 'Daily counts of active fine particulate matter (PM<sub>2.5</sub>) monitors in each Canadian province/territory. 
...
zone_summary_texts <- list(
  mean_map = 'Interactive map of seasonal mean PM<sub>2.5</sub> concentrations ({{< var units.pm >}}) by Canadian forecast zone. Hover over a zone for more information.',
  max_map = 'Interactive map of seasonal max PM<sub>2.5</sub> concentrations ({{< var units.pm >}}) by Canadian forecast zone. Hover over a zone for more information.',
  ts = 'Daily mean PM<sub>2.5</sub> concentrations ({{< var units.pm >}}) by Canadian forecast zone for each province/territory. The black line indicates the daily mean of the forcast zone averages.',
  zone_map = 'Note: Not all forecast zones have observation data. See `When / where did event occur?` for a daily record of forecast zones with data by province/territory'
)
...
worst_event_texts = paste0(
  "Hourly PM<sub>2.5</sub> concentrations from all FEM and PA monitors in the forecast zone for a selection of the worst events this season in ",
  prov_pretty,
  '. 
...
community_exceed_texts = list(
  exceeds_60 = 'Number of hours each community with nearby observation data had a mean PM<sub>2.5</sub> concentration above 60 {{< var units.pm >}} for each province/territory. A monitor must be within 25 km of a community and not closer to a different community to be considered located in that community.',
  exceeds_100 = 'Number of hours each community with nearby observation data had a mean PM<sub>2.5</sub> concentration above 100 {{< var units.pm >}} for each province/territory. A monitor must be within 25 km of a community and not closer to a different community to be considered located in that community.'
)
...
`r build_overview_card(type = "seasonal", report_dropdown = old_report_dropdown)`
...
`r sum(seasonal_zone_summaries$n != 0)` of the `r nrow(seasonal_zone_summaries)` forecast regions in Canada had observation data in the `r report_season` season. `r text$mean_counts$low` of these zones had a seasonal (6-month) zone mean above 10 {{< var units.pm >}}, `r text$mean_counts$mod` had a mean above 30 {{< var units.pm >}}, `r text$mean_counts$high` had a mean above 60 {{< var units.pm >}}, and `r text$mean_counts$vhigh` had a mean above 100 {{< var units.pm >}}.

`r text$max_counts$vhigh` of the `r nrow(seasonal_zone_summaries)` forecast regions in Canada experienced concentrations above 100 {{< var units.pm >}} during the `r report_season` season, `r text$max_counts$high` experienced concentrations above 60 {{< var units.pm >}}, and `r text$max_counts$mod` experienced concentrations above 30 {{< var units.pm >}}.

The most widespread impacts this season occurred in `r text$daily$prov_terr[1]`, `r text$daily$prov_terr[2]`, and `r text$daily$prov_terr[3]`, where daily forecast zone mean concentrations were `r paste(text$daily$mean[3], text$daily$mean[1], sep = " - ")` {{< var units.pm >}} on average.

The worst impacts this season occurred in `r text$daily2$prov_terr[1]`, `r text$daily2$prov_terr[2]`, and `r text$daily2$prov_terr[3]`, where daily forecast zone max concentrations were `r paste(text$daily$max[3], text$daily$max[1], sep = " - ")` {{< var units.pm >}} on average.

`r text$least$prov_terr[1]`, `r text$least$prov_terr[2]`, and `r text$least$prov_terr[3]` were the least impacted (where we had data), but still experienced daily mean concentrations as high as `r paste(text$least$max[1], text$least$max[3], sep = " - ")` {{< var units.pm >}}.
...
plot_paths$mean_map |>
  stringr::str_replace(report_dir_esc, "./") |>
  plot_card(zone_summary_texts$mean_map, iframe = TRUE, iframe_height = 625)
```
...
plot_paths$max_map |>
  stringr::str_replace(report_dir_esc, "./") |>
  plot_card(zone_summary_texts$max_map, iframe = TRUE, iframe_height = 625)
```
...
plot_paths$ts |>
  stringr::str_replace(report_dir_esc, "./") |>
  plot_card(zone_summary_texts$ts)
```
...
`r nrow(text$n_max_100)` communities of the `r nrow(pd)` in Canada with observation data in the `r report_season` season experienced concentrations in excess of 100 {{< var units.pm >}}. `r text$more_100_over_100$p[text$more_100_over_100=="Canada"]`% of communities with data experienced more than 100 hours in excess of 100 {{< var units.pm >}}; these communities were located in `r text$provs_over_100`. `r text$worst$community` a community in `r text$worst$prov_terr`, experienced the most hours above 100 {{< var units.pm >}} (`r text$worst$hours_exceeding_100` hours in total, out of `r text$worst$hours_w_obs` hours where observations were available).

`r nrow(text$n_max_60)` communities of the `r nrow(pd)` in Canada with observation data in the `r report_season` season experienced concentrations in excess of 60 {{< var units.pm >}}. `r text$more_60_over_100$p[text$more_60_over_100=="Canada"]`% of communities with data experienced more than 100 hours in excess of 60 {{< var units.pm >}}; these communities were located in `r text$provs_over_60`. `r text$worst_60$community` a community in `r text$worst_60$prov_terr`, experienced the most hours above 60 {{< var units.pm >}} (`r text$worst_60$hours_exceeding_100` hours in total, out of `r text$worst$hours_w_obs` hours where observations were available).
...
plot_paths$exceeds_100 |>
  stringr::str_replace(report_dir_esc, "./") |>
  plot_card(community_exceed_texts$exceeds_100, iframe = TRUE)
```
...
plot_paths$exceeds_60 |>
  stringr::str_replace(report_dir_esc, "./") |>
  plot_card(community_exceed_texts$exceeds_60, iframe = TRUE)
```
