...
text = list(
  most = table_data |> dplyr::mutate(t = FEM+PA) |> 
    dplyr::arrange(desc(t)) |> dplyr::pull(`
Prov. / Terr.
`) |>
    head(3),
  change = tmp |>
...
seasonal_zone_summaries = obs |>
  ...
  dplyr::mutate(
    ...
    label = paste0(
               ...
               "Seasonal mean PM<sub>2.5</sub>: <b>", handyr::swap(round(zone_mean_pm25,1), NA, with = "-"), " {{< var units.pm >}}</b><br>",
               "Seasonal max PM<sub>2.5</sub>: <b>", handyr::swap(round(zone_max_pm25,1), NA, with = "-"), " {{< var units.pm >}}</b>"
             ) |>
...
legend_title = paste0(report_season, "<br>Max PM<sub>2.5</sub> ({{< var units.pm >}})")
...
legend_title = paste0(report_season, "<br>Mean PM<sub>2.5</sub> ({{< var units.pm >}})")
pm_legend = paste0('<div style="margin-bottom:3px"><strong><span>',
                   legend_title, "</span></strong></div>",
                   '<i style="background:#ADADAD;opacity:1;"></i> No Data.<br>',
                   paste0('<i style="background:', leg_ugm3$colours,
...
` = reactable::colDef(
            ...
            columnGroups = list(
              reactable::colGroup(name = "", columns = c("Prov./Terr.")),
              reactable::colGroup(name = "Forecast Zone Mean PM2.5", 
                       columns = c("Mean", "Max", "98th Percentile",
                                   "Mean Hours >= 60", "Mean Hours >= 100")),
              reactable::colGroup(name = "Forecast Zone PM2.5 Events", 
...
table = reactable(
  ...
  columnGroups = list(
    reactable::colGroup(name = "Location", columns = c("Prov./Terr.", "Forecast Zone")),
    reactable::colGroup(
      name = "Event Timeline",
      columns = c(
        "Start",
    ...
    reactable::colGroup(name = "PM2.5 Monitors", columns = c("# of FEMs", "# of PAs")),
    reactable::colGroup(
...
`)) |>
  ...
  m_summary = paste0(
    "In ", month, " there was ",nrow(e_data)," detected events in Canada where the daily max PM<sub>2.5</sub> concentration in a forecast zone exceeded 30 {{< var units.pm >}} for more than 1 day in a row. The most events this month occurred in ", paste(e_prov_counts$`
Prov. / Terr.
`[1:2], collapse = ", ") |> paste("and", e_prov_counts$`
Prov. / Terr.
`[3])," (",paste(min(e_prov_counts$n), "-", max(e_prov_counts$n))," events). The longest events this month occurred in ",paste(e_prov_max_dur$`
Prov. / Terr.
`[1:2], collapse = ", ") |> paste("and", e_prov_max_dur$`
Prov. / Terr.
`[3]), " (max ",paste(min(e_prov_max_dur$dur), "-", max(e_prov_max_dur$dur))," days long). "
  )
...
`))
    ...
    if(nrow(p_m_data)==1 & p_m_data$aqhip[1] == "Low [0-30)"){
       paste0(
        "In ",prov," this month, no forecast zones with observations had a day exceeding 30 {{< var units.pm >}}."
       )
    }else{
      paste0(
        "In ",prov,", the worst day this month was ", format(p_m_data$worst_day[1], "%b %d"), ", where ", round(p_m_data$p[1]*100,1),"% of forecast zones had a maximum daily PM<sub>2.5</sub> concentration ", p_m_data$desc[1], " {{< var units.pm >}}."
      )
    }
...
table = reactable(
  ...
  # ),
  columnGroups = list(
    reactable::colGroup(name = "Location", columns = c("Community", "Prov./Terr.")),
    reactable::colGroup(name = "PM2.5 Monitors", columns = c("# of FEMs", "# of PAs")),
    reactable::colGroup(
      name = "Hours",
