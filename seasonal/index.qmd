...
title: "Canadian PM<sub>2.5</sub> Observations Seasonal Summary for <mark class='bg-info'>`r params$report$name_parsed`</mark>"
subtitle: | 
...
citation: 
  id: seasonal-report
  title: "Seasonal Canada PM2.5 Summary"
  url: https://aqmap.ca/aqmap/reports/seasonal
...
old_report_dropdown <- report_dir |>
  handle_old_reports(
    report_pattern = "[Summer,Winter]\\.html$",
...
## Analysis of Past 6-months of PM<sub>2.5</sub> Observations {.panel-tabset .tabset-dropdown style="display:none;"}

`r tab_explainers$primary_tabs`

### <big><strong>Where/when did we have observations?</strong></big> {.panel-tabset}
...
### <big><strong>How was this season overall?</strong></big> {.panel-tabset}
...
seasonal_zone_summaries = obs |>
  ...
  dplyr::mutate(
    ...
    label = paste0(
               ...
               "Seasonal mean PM<sub>2.5</sub>: <b>", handyr::swap(round(zone_mean_pm25,1), NA, with = "-"), " {{< pm_units >}}</b><br>",
               "Seasonal max PM<sub>2.5</sub>: <b>", handyr::swap(round(zone_max_pm25,1), NA, with = "-"), " {{< pm_units >}}</b>"
             ) |>
...
legend_title = paste0(report_season, "<br>Max PM<sub>2.5</sub> ({{< pm_units >}})")
...
legend_title = paste0(report_season, "<br>Mean PM<sub>2.5</sub> ({{< pm_units >}})")
pm_legend = paste0('<div style="margin-bottom:3px"><strong><span>',
                   legend_title, "</span></strong></div>",
                   '<i style="background:#ADADAD;opacity:1;"></i> No Data.<br>',
                   paste0('<i style="background:', leg_ugm3$colours,
...
# Define plot summary text (figure titles essentially)
plot_texts$mean_map = 'Interactive map of seasonal mean PM<sub>2.5</sub> concentrations ({{< pm_units >}}) by Canadian forecast zone. Hover over a zone for more information.'
plot_texts$max_map = 'Interactive map of seasonal max PM<sub>2.5</sub> concentrations ({{< pm_units >}}) by Canadian forecast zone. Hover over a zone for more information.'
...
# Define plot summary text (figure titles essentially)
plot_texts$ts = 'Daily mean PM<sub>2.5</sub> concentrations ({{< pm_units >}}) by Canadian forecast zone for each province/territory. The black line indicates the daily mean of the forcast zone averages.
...
`r sum(seasonal_zone_summaries$n != 0)` of the `r nrow(seasonal_zone_summaries)` forecast regions in Canada had observation data in the `r report_season` season. `r text$mean_counts$low` of these zones had a seasonal (6-month) zone mean above 10 {{< pm_units >}}, `r text$mean_counts$mod` had a mean above 30 {{< pm_units >}}, `r text$mean_counts$high` had a mean above 60 {{< pm_units >}}, and `r text$mean_counts$vhigh` had a mean above 100 {{< pm_units >}}.

`r text$max_counts$vhigh` of the `r nrow(seasonal_zone_summaries)` forecast regions in Canada experienced concentrations above 100 {{< pm_units >}} during the `r report_season` season, `r text$max_counts$high` experienced concentrations above 60 {{< pm_units >}}, and `r text$max_counts$mod` experienced concentrations above 30 {{< pm_units >}}.

The most widespread impacts this season occurred in `r text$daily$prov_terr[1]`, `r text$daily$prov_terr[2]`, and `r text$daily$prov_terr[3]`, where daily forecast zone mean concentrations were `r paste(text$daily$mean[3], text$daily$mean[1], sep = " - ")` {{< pm_units >}} on average.

The worst impacts this season occurred in `r text$daily2$prov_terr[1]`, `r text$daily2$prov_terr[2]`, and `r text$daily2$prov_terr[3]`, where daily forecast zone max concentrations were `r paste(text$daily$max[3], text$daily$max[1], sep = " - ")` {{< pm_units >}} on average.

`r text$least$prov_terr[1]`, `r text$least$prov_terr[2]`, and `r text$least$prov_terr[3]` were the least impacted (where we had data), but still experienced daily mean concentrations as high as `r paste(text$least$max[1], text$least$max[3], sep = " - ")` {{< pm_units >}}.
...
### <big><strong>When/where did events occur?</strong></big> {.panel-tabset}
...
`)) |>
  ...
  m_summary = paste0(
    "In ", month, " there was ",nrow(e_data)," detected events in Canada where the daily max PM<sub>2.5</sub> concentration in a forecast zone exceeded 30 {{< pm_units >}} for more than 1 day in a row. The most events this month occurred in ", paste(e_prov_counts$`
Prov. / Terr.
...
`))
    ...
    if(nrow(p_m_data)==1 & p_m_data$aqhip[1] == "Low [0-30)"){
       paste0(
        "In ",prov," this month, no forecast zones with observations had a day exceeding 30 {{< pm_units >}}."
       )
    }else{
      paste0(
        "In ",prov,", the worst day this month was ", format(p_m_data$worst_day[1], "%b %d"), ", where ", round(p_m_data$p[1]*100,1),"% of forecast zones had a maximum daily PM<sub>2.5</sub> concentration ", p_m_data$desc[1], " {{< pm_units >}}."
      )
    }
...
### <big><strong>What were the worst events like?</strong></big> {.panel-tabset}
...
texts = sapply(prov_pretty, \(prov) {
  ...
  if (nrow(worst) == 0) {
    ...
    paste0(
      ...
      " {{< pm_units >}}, respectively. The longest event (",
      worst_summs$dur_max,
      ...
      " {{< pm_units >}} and an overall mean of ",
      worst_summs$dur_mean,
      " {{< pm_units >}} was observed."
    )
  }
})
...
### <big><strong>Which communities [with data] were impacted?</strong></big> {.panel-tabset}
...
# Define plot summary text (figure titles essentially)
plot_texts = list(
  exceeds_60 = 'Number of hours each community with nearby observation data had a mean PM<sub>2.5</sub> concentration above 60 {{< pm_units >}} for each province/territory. A monitor must be within 25 km of a community and not closer to a different community to be considered located in that community.',
  exceeds_100 = 'Number of hours each community with nearby observation data had a mean PM<sub>2.5</sub> concentration above 100 {{< pm_units >}} for each province/territory. A monitor must be within 25 km of a community and not closer to a different community to be considered located in that community.'
)
...
`r nrow(text$n_max_100)` communities of the `r nrow(pd)` in Canada with observation data in the `r report_season` season experienced concentrations in excess of 100 {{< pm_units >}}. `r text$more_100_over_100$p[text$more_100_over_100=="Canada"]`% of communities with data experienced more than 100 hours in excess of 100 {{< pm_units >}}; these communities were located in `r text$provs_over_100`. `r text$worst$community` a community in `r text$worst$prov_terr`, experienced the most hours above 100 {{< pm_units >}} (`r text$worst$hours_exceeding_100` hours in total, out of `r text$worst$hours_w_obs` hours where observations were available).

`r nrow(text$n_max_60)` communities of the `r nrow(pd)` in Canada with observation data in the `r report_season` season experienced concentrations in excess of 60 {{< pm_units >}}. `r text$more_60_over_100$p[text$more_60_over_100=="Canada"]`% of communities with data experienced more than 100 hours in excess of 60 {{< pm_units >}}; these communities were located in `r text$provs_over_60`. `r text$worst_60$community` a community in `r text$worst_60$prov_terr`, experienced the most hours above 60 {{< pm_units >}} (`r text$worst_60$hours_exceeding_100` hours in total, out of `r text$worst$hours_w_obs` hours where observations were available).
