...
load_report_data <- function(min_date, max_date, max_pm25_allowed) {
  db <- connect_to_db()
  ...
  fem_obs_query <- db |>
    handyr::read_from_database(
      table_name = tbls$FEM$obs,
      collect = FALSE,
      query_fun = \(df) {
        df |>
          dplyr::select(site_id, date, pm25) |>
      ...
      query_fun = \(df) {
        df |>
          ...
          dplyr::group_by(
            site_id,
            date = dplyr::sql(
              "TO_TIMESTAMP(CEIL(EXTRACT(EPOCH FROM date) / 3600) * 3600)"
            )
          ) |>
          dplyr::summarise(
            pm25 = mean(pm25, na.rm = TRUE),
            pm25_a = mean(pm25_a, na.rm = TRUE),
            pm25_b = mean(pm25_b, na.rm = TRUE),
            temperature = mean(temperature, na.rm = TRUE),
            rh = mean(rh, na.rm = TRUE),
            .groups = "drop"
          ) |>
          dplyr::mutate(monitor = "PA") |>
          dplyr::union_all(fem_obs_query)
      }
    ) |>
    dplyr::ungroup() # TODO: is this needed?
  ...
  fem_meta <- db |>
    handyr::read_from_database(
      table_name = tbls$FEM$meta,
      query_fun = \(df) {
        df |>
          dplyr::select(dplyr::all_of(desired_cols)) |>
          dplyr::filter(
            site_id %in%
              !!(unique(dplyr::filter(obs, monitor == "FEM")$site_id))
          ) |>
          dplyr::filter(prov_terr != "External" & prov_terr != "United States")
      }
    )
    ...
    handyr::read_from_database(
      ...
      query_fun = \(df) {
        df |>
          dplyr::select(dplyr::all_of(desired_cols)) |>
          dplyr::filter(
            site_id %in% !!(unique(dplyr::filter(obs, monitor == "PA")$site_id))
          ) |>
          dplyr::filter(
            prov_terr != "External" &
              prov_terr != "United States" &
              prov_terr != "Canada_Buffer"
          )
      }
    ) |>
    dplyr::mutate(monitor = "PA") |>
    dplyr::bind_rows(
      fem_meta |> dplyr::mutate(monitor = "FEM", is_aqsu = FALSE)
    )
  ...
  obs <- obs |>
    dplyr::filter(
      paste(monitor, site_id) %in% paste(meta$monitor, meta$site_id)
    )

  obs |>
    dplyr::left_join(meta, by = c("site_id", "monitor")) |>
    dplyr::mutate(prov_terr = factor(prov_terr, prov_pretty)) |>
    dplyr::mutate(pm25 = ifelse(pm25 > max_pm25_allowed, NA, pm25))
}
