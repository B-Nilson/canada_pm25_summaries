...
donuts_caption <- paste(
  "Distribution of monthly median concentrations at %s PM2.5 monitoring sites within each p/t.",
  "The number within each circle indicates the total number of monitors with data in that area.",
  "Percentages indicate the amount of those monitors with a monthly median within that concentration range."
) |>
...
quarto_captions <- list(
  median_donuts = donuts_caption,
  max_donuts = donuts_caption |> stringr::str_replace_all(" median ", " max "),
  coverage = paste(
  ...
  boxplots = paste0(
    "Each boxplot summarises the distribution of monthly mean %s monitoring site PM2.5 concentrations for that p/t for %s.",
    "Blue points for individual sites are displayed for any monitors higher than that regions 75th percentile.",
    "Hover over the blue points (if present) to see details about that specific site; hover over the boxplot (away from the points) to see the min/median/max and 25th/75th percentiles for that p/t.",
    "See the menu in the top right of the plot for more."
  ) |>
    sprintf(names(monitor_group), fig_caption_date_range) |>
    setNames(monitor_groups),
  daily_zones = paste(
  ...
  monthly_zones = paste(
    ...
    sprintf(fig_caption_date_range),
  prov_grids = paste(
    "Median and maximum daily PM2.5 concentrations for all %s monitoring sites in each p/t.",
    "The median is useful for identifying large scale impacts across the p/t,",
    "while the maximum can be used to identify impacts in at least one area of the p/t."
  ) |>
    sprintf(names(monitor_groups)) |>
    setNames(monitor_groups),
  zone_grids = paste(
    "Daily median PM2.5 concentrations for all FEM and PA monitors within each forecast zone in %s for %s.",
...
summaries$donuts$paths$median_fem_only |>
  stringr::str_replace(report_dir_esc, "./") |>
  plot_card(quarto_captions$median_donuts$`FEM Only`)
```

```{r include=TRUE, results = 'asis'}
summaries$donuts$paths$max_fem_only |>
  stringr::str_replace(report_dir_esc, "./") |>
  plot_card(quarto_captions$max_donuts$`FEM Only`)
```
...
summaries$donuts$paths$median_pa_only |>
  stringr::str_replace(report_dir_esc, "./") |>
  plot_card(quarto_captions$median_donuts$`PA Only`)
```

```{r include=TRUE, results = 'asis'}
summaries$donuts$paths$max_pa_only |>
  stringr::str_replace(report_dir_esc, "./") |>
  plot_card(quarto_captions$max_donuts$`PA Only`)
```
...
plot_paths[['FEM']] |>
  stringr::str_replace(report_dir_esc, "./") |>
  plot_card(quarto_captions$prov_grids[['FEM Only']])
```
...
plot_paths[['PA']] |>
  stringr::str_replace(report_dir_esc, "./") |>
  plot_card(quarto_captions$prov_grids[['PA Only']])
```
...
plot_paths[["FEM"]] |>
  stringr::str_replace(report_dir_esc, "./") |>
  plot_card(quarto_captions$boxplots[["FEM Only"]], iframe = TRUE)
```
...
plot_paths[["PA"]] |>
  stringr::str_replace(report_dir_esc, "./") |>
  plot_card(quarto_captions$boxplots[["PA Only"]], iframe = TRUE)
```
...
plot_paths[["FEM Only"]] |>
  stringr::str_replace(report_dir_esc, "./") |>
  plot_card(quarto_captions$community_boxplots[["FEM Only"]], iframe = TRUE)
```
...
plot_paths[["PA Only"]] |>
  stringr::str_replace(report_dir_esc, "./") |>
  plot_card(quarto_captions$community_boxplots[["PA Only"]], iframe = TRUE)
```
