...
summaries <- obs |>
  make_summaries(type = "monthly", fcst_zones = fcst_zones, meta_cols = meta_cols)
...
map = make_anim_map(summaries$daily_by_zone, legend_title,
                    height = 600)
...
map = make_sync_map(summaries$monthly_by_zone, 
...
pd = summaries$overall |>
  dplyr::mutate(monitor2 = ifelse(monitor == "FEM", "FEM Only",
                           ifelse(monitor == "PA","PA Only",
                                  "FEM and PA"))) |>
  dplyr::bind_rows(summaries$overall |> dplyr::mutate(monitor2 = "FEM and PA")) |>
...
table =  reactable(data = summaries$worst_day,
                   pagination= FALSE,
