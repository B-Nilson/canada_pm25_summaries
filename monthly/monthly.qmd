...
# Handle report end time - 
date_range = get_report_start_end(report_dir, "monthly", run_future_month)
min_date = date_range[1]
max_date = date_range[2]

hours_to_summarise = seq(min_date, max_date, "1 hours")

caption_range = paste(min_date|> format("%b %d, %Y %H:%M UTC"), "to",
                      max_date|> format("%b %d %H:%M UTC"))
plot_captions = paste("Obs. from Canadian", names(monitor_groups), 
                      "monitors from", caption_range) |>
  setNames(names(monitor_groups)) |> as.list()
```

```{r loadData, cache=cache }

# Load data from rds from last run if exists, if not, pull from database and create RDS
rds_copy = file.path(report_dir, "data", format(min_date, "%Y_%m.rds"))
...
.tmp = make_summaries(obs, type = "monthly")
      ...
      \(m){
          ...
          pd[[stat]] |> 
            make_donut_plot(labels, m |> stringr::str_remove(" Only"), 
                            stringr::str_to_title(stat), format(max_date, "%b.")) |> 
...
There was <strong>`r sum(p_fem$n)` FEM sites</strong> and <strong>`r sum(p_pa$n)` PA sites</strong> reporting PM<sub>2.5</sub> in Canada in `r format(max_date, "%B %Y")`. <strong>
...
plot_text = paste(
  ...
  "during the month of", format(max_date, "%B, %Y."),
...
plot_title = paste0("Animated Map of Daily Forecast Zone Mean PM<sub>2.5</sub> for ",format(max_date,"%B %Y"))
plot_path = paste0(report_dir, "plots/fcstzone_mean_map_daily_", 
                    format(max_date,"%Y-%m"),".html")
...
plot2_title = paste0("Map of Forecast Zone Mean [left] and Max [right] for ",format(max_date,"%B %Y"))
plot2_path = paste0(report_dir, "plots/fcstzone_mean_map_", 
                    format(max_date,"%Y-%m"),".html")

legend_titles = list()
legend_titles$pm25 = legend_title |>
    stringr::str_replace("Daily", format(max_date,"%b."))
...
There was <strong>`r aqhi_p_counts$FEM[4]` FEM monitors</strong> and <strong>`r aqhi_p_counts$PA[4]` PA monitors</strong> in Canada with a mean PM<sub>2.5</sub> concentration for `r format(max_date, "%B %Y")` <strong>exceeding 100 &mu;g m<sup>-3</sup></strong>
...
There was <strong>`r aqhi_p_counts$FEM[3]` FEM monitors</strong> and <strong>`r aqhi_p_counts$PA[3]` PA monitors</strong> in Canada with a mean PM<sub>2.5</sub> concentration for `r format(max_date, "%B %Y")` <strong>between 60 and 100 &mu;g m<sup>-3</sup></strong>
...
.tmp = names(monitor_groups) |> lapply(\(m){
  ...
  ggs = lapply(names(pd), \(stat){
    pd[[stat]] |>
      make_grid_plot(xlab = paste("Day of", format(max_date, "%B")),
...
#   data.frame() |>
#   dplyr::bind_rows(fcst_zones@data |> subset(PROVINCE_C |> stringr::str_detect(","))) |>
...
# 
# pd2 = obs |>
#   dplyr::group_by(y = fcst_zone, x= lubridate::day(date)|>factor(unique(lubridate::day(date))), z = prov_terr, monitor)|>
#   dplyr::summarise(fill = quantile(pm25,0.5,na.rm=TRUE) |> aqhi_p()) |>
#   dplyr::full_join(expand.grid(y = obs$fcst_zone |> unique(), 
#                         x = day_order |>factor(day_order)) |>
#               dplyr::left_join(subset(obs, !duplicated(fcst_zone)) |>
#                           dplyr::select(y = fcst_zone, z = prov_terr), by = "y"), by = c('y','x','z')) |>
#   dplyr::group_by(monitor, z,y) |>
#   dplyr::summarise(n = n()) |>
#   dplyr::group_by(monitor, prov = z) |>
#   dplyr::summarise(n_fem_only = n()) |> subset(monitor == "FEM")
# 
# p = pd |>
...
# 
# make_text = function(p, province){
#   tmp = p |> subset(prov == province)
#   tmp$p = ifelse(is.na(tmp$n_fem_only),"0%", tmp$p)
#   tmp$p_fem = ifelse(is.na(tmp$n_fem_only),"0%", tmp$p_fem)
#   
#   paste(htmltools::tags$strong(tmp$p),"of the",htmltools::tags$strong(tmp$total_zones), "forecast zones in", province, "have a FEM or PA reporting data this month. Without PA monitors", htmltools::tags$strong(tmp$p_fem), "of the forecast zones have data.")
# }
...
`r vals[[1]]$C` (a community in `r vals[[1]]$P` with `r vals[[1]]$NFEM` FEM and `r vals[[1]]$NPA` PA monitors within at least `r vals[[1]]$D` km) had the highest observed hourly PM<sub>2.5</sub> concentration in Canada for `r format(max_date, "%B %Y")` (`r vals[[1]]$PM2` &mu;g m<sup>-3</sup>). During this period, `r vals[[2]]$C` (a community in `r vals[[2]]$P` with `r vals[[2]]$NFEM` FEM and `r vals[[2]]$NPA` PA monitors within at least `r vals[[2]]$D` km) had the highest 1-month mean in Canada (`r round(vals[[2]]$PM1,1)` &mu;g m<sup>-3</sup>).
...
dat1 = obs_summary |>
  ...
  dplyr::left_join(fcst_zones |> 
              dplyr::select(fcst_zone, zone_prov = prov_terr), 
            by = "fcst_zone") |>
...
dl = dat1 |>
  ...
  dplyr::mutate("Day(s) of Month w/ 12+ hours Above 100 μg/m3" = paste0("'",`Day(s) of Month w/ 12+ hours Above 100 μg/m3`)) |>
   download_this(
    output_name = "fcst_zone_pm25_summaries_" |> 
      paste0(max(obs$date) |> format("%Y-%m")),
