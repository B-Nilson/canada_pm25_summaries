...
summaries <- obs |> make_summaries(type = "monthly", fcst_zones = fcst_zones)
...
map = make_anim_map(summaries$zone_sum_daily, legend_title,
                    height = 600)
...
map = make_sync_map(summaries$zone_sum_month, 
...
# Get data for site mean boxplots
pd = summaries$obs_summary2 |>
  dplyr::mutate(monitor2 = ifelse(monitor == "FEM", "FEM Only",
                           ifelse(monitor == "PA","PA Only","FEM and PA"))) |>
  dplyr::bind_rows(summaries$obs_summary2 |> dplyr::mutate(monitor2 = "FEM and PA") |>
...
dl = summaries$worst_day_df |>
  setNames(names(summaries$worst_day_df) |> stringr::str_replace_all("\n"," ")) |>
   download_this(
...
table =  reactable(data = summaries$worst_day_df,
                   pagination= FALSE,
